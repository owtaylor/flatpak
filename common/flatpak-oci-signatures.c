                                    ...
                                    GError              **error)
{
  ...
  for (i = 1; i < self->signatures->len; i++)
    {
      g_autofree char *filename = g_strdup_printf ("signature-%u", i);
      gconstpointer data;
      gsize size;

      data = g_bytes_get_data (self->signatures->pdata[i], &size);
  ...
  if (g_once_init_enter_pointer (&regex))
    {
      g_autoptr(GRegex) compiled = g_regex_new (STRIP_TAG_AND_DIGEST_RE,
                                                ...
                                                NULL);
      g_once_init_leave_pointer (&regex, g_steal_pointer (&compiled));
    }

  matched = g_regex_match (regex, str, G_REGEX_MATCH_DEFAULT, &match_info);
  g_assert (matched);
                               ...
                               GError              **error)
{
  ...
  expected_identity = g_strdup_printf ("%s/%s", g_uri_get_host(uri), repository_name);

  guint i;
  for (i = 0; i < self->signatures->len; i++)
    {
      ...
      if (signature->critical.identity.reference != NULL)
        {
          g_autofree char *stripped = reference_strip_tag_and_digest (signature->critical.identity.reference);

          if (strcmp (expected_identity, stripped) != 0)
            {
      ...
      g_info ("%s: found valid signature for %s@%s", remote_name, expected_identity, digest);
      return TRUE;
    }
