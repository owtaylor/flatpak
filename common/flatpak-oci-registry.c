...
/*
 * This is a small helper to handle cleanup for the temporary directory
...
temporary_directory_new (GError **error)
{
  ...
  self->path = g_build_filename ("/var/tmp", "oci-XXXXXX", NULL);
  if (!g_mkdtemp (self->path))
    return glnx_null_throw_errno_prefix (error, "mkdtemp");
                       ...
                       GError               **error)
{
  ...
  else if (strcmp (manifest_ref, ref) != 0)
    {
